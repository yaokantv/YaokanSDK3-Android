...
public class SelectProviderActivity extends BaseActivity implements View.OnClickListener, YaokanSDKListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setMTitle(R.string.title_select_provider, TITLE_LOCATION_CENTER);
        reload();
    }
    ...
    protected void initView() {
        ...
        provinceAdapter = new CommonAdapter<Area>(this, provinceList, R.layout.inner_item_text_lv) {
            @Override
            public void convert(ViewHolder helper, Area item, int position) {
                ...
                if (position == proIndex) {
                    ...
                    helper.setBgResource(R.id.ll_item_bg, R.color.provider_bg);
        ...
        cityAdapter = new CommonAdapter<Area>(this, cityList, R.layout.inner_item_text_lv) {
            @Override
            public void convert(ViewHolder helper, Area item, int position) {
                helper.setText(R.id.tv, item.getName());
                TextView textView = helper.getView(R.id.tv);
        ...
        providerAdapter = new CommonAdapter<Operators>(this, providerList, R.layout.inner_item_text_lv) {
            @Override
    ...
    public void onClick(View v) {
        if (v.getId() == R.id.yk_next) {
            if (providerList.size() > mPosition) {
                ...
                intent.putExtra(Config.S_TID,1);
                intent.putExtra(Config.S_BID,operators.getBid());
                bundle.putParcelable(Config.S_OPE,operators);
